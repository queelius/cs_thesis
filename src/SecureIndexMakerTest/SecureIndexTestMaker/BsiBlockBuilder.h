...
namespace alex { namespace index
{
    ...
    public:
        ...
        SecureIndex build()
        {
            ...
            // should actulaly be: 1 - (1 - fp_rate)^k, but are very close for reasonable values of fp_rate
            params.false_positive_probability = _params.falsePositiveRate / (double)_numBlocks;

            for (size_t i = 0; i < _numBlocks; ++i)
            {
                if (_params.noiseFactor > 0)
                {
                    size_t realCount = _blocks[i].size() * _params.secrets.size();
                    size_t junkCount = (size_t)std::round(_params.noiseFactor * (_blocks[i].size() * _params.secrets.size()));
                    params.projected_element_count = realCount + junkCount;
                    params.random_seed = entropy.get();
                    params.compute_optimal_parameters();

                    bsi->_blooms.push_back(bf::bloom_filter(params));

                    // this is not an optimal way to add noise;
                    // should be able to randomly modify bit vector elements directly
                    // without using any hash functions, but that interface is not
                    // exposed to me and i don't want to risk damaging anything.
                    //
                    // i won't use any timing measurements for this, or if i do
                    // take it with a grain of salt
                    for (int x = 0; x < junkCount; ++x)
                    {
                        bsi->_blooms[i].insert(alex::utils::generateId(8));
                    }
