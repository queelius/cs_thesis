...
namespace alex { namespace utils
{
    ...
    std::vector<std::string> cryptoTerms(std::string term, const std::vector<std::string>& secrets, const std::string& salt)
    {
        std::vector<std::string> results;
        results.reserve(secrets.size());
        for (const auto& secret : secrets)
        {
            std::string stage1 = alex::crypt::hexdigest<alex::globals::digest_size>(term + "|" + secret);
            std::string stage2 = alex::crypt::hexdigest<alex::globals::digest_size>(stage1 + "|" + salt);
            results.push_back(stage2);
        }
        return results;
    }
