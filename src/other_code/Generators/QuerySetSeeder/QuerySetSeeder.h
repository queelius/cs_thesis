...
namespace alex { namespace corpus
{
    ...
    public:
        struct Params
        {
            std::vector<alex::index::Query> queries;
                           ...
                           bool verbose = false)
        {
            ...
            for (auto file : files)
            {
                try
                {
                    ...
                    std::vector<std::pair<alex::index::Query::Term, size_t>> seedPos =
        ...
        template <class T>
        std::vector<std::pair<alex::index::Query::Term, size_t>> getSeedPositions(const std::vector<std::string>& doc, alex::stochastic::Entropy<T>& entropy,
                                                                                  bool verbose = false)
        {
            namespace q = alex::index;
            size_t docSize = doc.size();
            std::vector<std::pair<q::Query::Term, size_t>> positions;
            ...
            for (auto query : _params.queries)
            {
                ...
                {
                    float p = entropy.getReal(_params.maxBinomial, _params.maxBinomial);
                    if (entropy.get0_1() >= p)
                        continue;
            ...
            std::sort(positions.begin(), positions.end(), [](
                const std::pair<q::Query::Term, size_t>& x1, const std::pair<q::Query::Term, size_t>& x2)
            {
