...
namespace alex { namespace index
{
    ...
    public:
        ...
        {
            // finish this
            std::cout << this->_maxFreq << std::endl;
            std::cout << this->_ref << std::endl;
        };
        ...
        SecureIndex build()
        {
            ...
            //psi->_approximateWords = _words.size() + f.getInt(-_params.approximateWordError, _params.approximateWordError);
            psi->_freqWidth = (uint32_t)std::ceil(std::log(_maxFreq) / std::log(2));
            psi->_freqBits.resizeBits(sz * psi->_freqWidth);
            // psi->_freqBits.resizeBits(sz * psi->_freqWidth + f.getInt(0, _params.freqBitsRandomPadding))
