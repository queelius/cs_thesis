    ...
    public:
        struct BuilderParams
        {
            ...
            void defaults()
            {
                ...
                if (verbose >= 1)
                {
                    std::cout << "PsiBlockBuilder::BuilderParams" << std::endl;
                    std::cout << "\tloadFactor: " << loadFactor << std::endl;
                    std::cout << "\twordsPerBlock: " << wordsPerBlock << std::endl;
                    std::cout << "\tmaxBlocks: " << maxBlocks << std::endl;
                    std::cout << "\tsecrets:" << std::endl;
                    for (const auto& secret : secrets)
                        std::cout << secret << " ";
                    std::cout << "\tfalsePositiveBits: " << falsePositiveBits << std::endl;
                    std::cout << "\tstemming: " << stemming << std::endl;
                    std::cout << "\tignoreWords: ";
                    if (ignoreWords.empty())
                        std::cout << "none" << std::endl;
                    else
                    {
                        for (const auto& word : ignoreWords)
        ...
        SecureIndex build()
        {
            ...
            //psi->_approximateWords = std::min(1, _approximateWords + f.getInt(-_params.approximateWordError, _params.approximateWordError));
            psi->_blockBits.resizeBytes(alex::utils::toBytes(sz * _numBlocks));
            // psi->_blockBits.resizeBytes(alex::utils::toBytes(sz * _numBlocks) + f.getInt(0, _params.blockBitsPadding))
            ...
            // perform a check to see if there are any extra positions
            // extras are good for preventing info leakage if bits
            // are randomized, but if no extras no reason to do this.
            if (_params.loadFactor < 1 && _params.algo != cmph_cpp::CMPH_CHD)
            {
