...
int main(int ac, char* av[])
{
    try
    {
        ...
        desc.add_options()
            ...
            ("load_trials", po::value<unsigned int>(&loadTrials)->default_value(loadTrials))
        ...
        for (auto type : types)
        {
            try
            {
                ...
                if (type == "psim")
                {
                    ...
                    //disable concurrency for psimin -- due to memory issues -- just change concurrency to false, below
