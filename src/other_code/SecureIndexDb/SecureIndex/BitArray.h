...
namespace alex { namespace collections
{
    ...
    void BitArray::read(std::istream& file)
    {
        ...
        _size = (unsigned int)alex::io::binary::readVarU32(file);
        _bits = new unsigned char[BITS_TO_CHARS(_size)];

        file.read((char*)_bits, BITS_TO_CHARS(_size));
    }

    void BitArray::write(std::ostream& file) const
    {
        alex::io::binary::writeVarU32(file, (uint32_t)_size);
        file.write((char*)_bits, bytes());
    }
    ...
    BitArray BitArray::makeBits(unsigned int bits, bool val)
    {
        ...
        std::fill_n(b._bits, bytes, val ? UCHAR_MAX : 0);
