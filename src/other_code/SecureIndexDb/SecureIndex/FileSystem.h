...
namespace alex { namespace io { namespace filesystem
{
    ...
    std::vector<boost::filesystem::path> getFiles(const boost::filesystem::path& p, std::string filePattern = ".*", uint32_t recursionDepth = 1);
    bool makeDirectory(const boost::filesystem::path& p);

    std::vector<boost::filesystem::path> getFiles(const boost::filesystem::path& p, std::string filePattern, uint32_t recursionDepth)
    {
    ...
    size_t getDirectorySize(const boost::filesystem::path& p, std::string filePattern, size_t recursionDepth)
    {
        size_t totalSize = 0;
