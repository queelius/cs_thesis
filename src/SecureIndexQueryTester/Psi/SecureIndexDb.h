    ...
    public:
        ...
        std::unordered_map<SecureIndex, double> bm25(const HiddenQuery& q)
        {
            ...
            for (const auto& t : q.terms)
            {
                ...
                for (const auto& si : _secureIndexes)
                {
                    ...
                    const double other = tf * (_params.bm25_k1 + 1) / (tf + _params.bm25_k1 * (1 - _params.bm25_b + _params.bm25_b * (si->getApproximateWords() / avgDL)));
                    totals[si] += other * idf;
                }
            }
            return totals;
        ...
        std::unordered_map<SecureIndex, double> minPairwiseDistanceScore(const HiddenQuery& q)
        {
            ...
            for (const auto& si : _secureIndexes)
            {
                ...
                {
                    minDist[si] = getMinPwScore(si->getMinPairwiseScore(q));
                }
                /*else
                {
        ...
        double getMinPwScore2(double d)
        {
            std::log(_params.alpha_min_pair + std::exp(-_params.beta_min_pair * d));
        };

        double getMinPwScore(double d)
        {
            
            // _params.alpha_min_pair == 0.3
            // _params.beta_min_pair == 0.01
            // _params.rho_min_pair = 3
            // _params.theta_min_pair = 1
            //
            // log(alpha + rho * exp(theta - beta * d))
            // log(0.3 + 3*e^(1-0.01d)
            // log(0.3 + 3e*e^(-0.01*d)
            double r = std::log(_params.alpha_min_pair + 3 * std::exp((100.0 - d) / 100.0));
            return (r < 0 ? 0 : r);
        };
        ...
        std::unordered_map<SecureIndex, double> getRanking2(const HiddenQuery& hq)
        {
            if (hq.terms.empty())
                throw std::exception("No Terms");

            std::unordered_map<SecureIndex, double> results;

            auto termRel = bm25(hq);
            if (hq.terms.size() == 1)
            {
                for (const auto& si : _secureIndexes)
                    results[si] = termRel[si];
            }
            else
            {
                for (const auto& si : _secureIndexes)
                {
                    if (si->hasProximity())
                        results[si] = _params.alpha_weighted_avg * termRel[si] +
                        (1 - _params.alpha_weighted_avg) * getMinPwScore(si->getMinPairwiseScore(hq));
                    else
                        results[si] = termRel[si];
                }
            }
