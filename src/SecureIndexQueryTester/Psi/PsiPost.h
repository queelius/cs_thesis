        ...
        public:
            ...
            std::vector<uint32_t> getLocations(const alex::index::HiddenQuery::HiddenTerm& t) const
            {
                ...
                if (t.size() == 1)
                {
                    ...
                    return (_hashBits.get(_hashWidth * idx, _hashWidth) == value ? _postings[idx] : results);
                }
                ...
                std::vector<uint32_t> path(postings.size());
                std::vector<std::set<uint32_t>> used(postings.size());
                alex::utils::cover2(postings, 0, std::numeric_limits<uint32_t>::max(),
                                    0, results, 2*_maxOffset + t.size(), max, path, used);
            ...
            /*
            std::vector<uint32_t> getLocations2(const alex::index::HiddenQuery::HiddenTerm& t) const
            {
                std::vector<uint32_t> results;
                if (t.empty())
                    return results;
                ...
                alex::utils::cover2(postings, 0, std::numeric_limits<uint32_t>::max(),
                                    0, candidates, 2*_maxOffset + t.size(), max);

                //for (const auto& p : candidates)
                //    results.push_back((p.first + p.second) / 2);
                //return results;
                std::unordered_set<uint32_t> results2;
                for (const auto& p : candidates)
                {
                    for (unsigned int i = p.first; i <= p.second; ++i)
                        results2.insert(i);
                }
                return std::vector<uint32_t>(results2.begin(), results2.end());
            };*/
            ...
            /*unsigned int getFrequency2(const alex::index::HiddenQuery::HiddenTerm& t) const
            {
                if (t.empty())
                    return 0;
                ...
                if (t.size() == 1)
                {
                    ...
                    const uint32_t idx = _ph.search(q);
                    const uint32_t value = alex::hash::jenkinsHash(q) % _maxHash;
                    return (_hashBits.get(_hashWidth * idx, _hashWidth) == value ? _postings[idx].size() : 0);
                }
                ...
                // maximum size of a term of a given size with
            ...
            unsigned int getFrequency(const alex::index::HiddenQuery::HiddenTerm& t) const
            {
                ...
                const std::string& suffix = "|" + _ref;
                unsigned int freq = std::numeric_limits<unsigned int>::max();
                for (const auto& x : t)
                {
                    const std::string& q = alex::crypt::hexdigest<alex::globals::digest_size>(x + suffix);
                    const unsigned int idx = _ph.search(q);
                    const uint32_t value = alex::hash::jenkinsHash(q) % _maxHash;
                    if (_hashBits.get(_hashWidth * idx, _hashWidth) != value)
                        return 0;
