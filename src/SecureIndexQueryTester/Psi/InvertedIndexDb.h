...
namespace alex { namespace index
{
    ...
    public:
        ...
        std::unordered_map<Index, double> minPairwiseDistanceScore(const Query& q)
        {
            ...
            for (const auto& idx : _indexes)
            {
                if (q.terms.size() == 1)
                    ...
                    minDist[idx] = getMinPwScore(idx->getMinPairwiseScore(q));
            }
        ...
        std::unordered_map<Index, double> getRanking2(const Query& q)
        {
            ...
            {
                for (const auto& idx : _indexes)
                    results[idx] = _params.alpha_weighted_avg * termRel[idx] +
                    (1 - _params.alpha_weighted_avg) * getMinPwScore(idx->getMinPairwiseScore(q));
            }
        ...
        double getMinPwScore(double d)
        {
            ...
            // log(alpha + rho * exp(theta) * exp(-beta * d)
            // log(0.3 + 3*e^(1-0.01d)
            // log(0.3 + 3e*e^(-0.02*d)
