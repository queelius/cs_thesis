    ...
    class PsiBlock: public ISecureIndex
    {
        ...
        public:
            ...
            void write(std::ostream& file)
            {
                ...
                _hashBits.writeBinary(file);

                // write block data
                writeVarU32(file, _blockWidth);
                writeVarU32(file, _blockSize);
                _blockBits.writeBinary(file);
            };

            bool blocks(const alex::index::HiddenQuery::HiddenTerm& t, alex::collections::BitArray& blocks) const
            {
                ...
                for (const auto& x : t)
                {
                    const std::string& q = alex::crypt::hexdigest<10>(x + suffix);

                    const uint32_t idx = _ph.search(q);
                    const uint32_t value = alex::fast_hash::jenkinsHash(q) % _maxHash;
            ...
            {
                auto blocks = alex::collections::BitArray::makeBits(_blockWidth, true);
                const std::string suffix = "|" + _ref;
            ...
            bool contains(const alex::index::HiddenQuery::HiddenTerm& t) const
            {
                ...
                if (t.size() == 1)
                {
                    const std::string q = alex::crypt::hexdigest<10>(t[0] + suffix);
                    const uint32_t idx = _ph.search(q);
                    const uint32_t value = alex::fast_hash::jenkinsHash(q) % _maxHash;
